...
export const getBalance = async (req, res) => {
    try {
        // The auth middleware provides req.userId from the access token
...
export const transfer = async (req, res) => {
    ...
    try {
        ...
        await Account.updateOne({ userId: fromUserId }, { $inc: { balance: -transferAmount } }).session(session);
...
export const getHistory = async (req, res) => {
    try {
        ...
        .populate("senderId", "fullname") // Get the sender's full name
        .populate("receiverId", "fullname") // Get the receiver's full name
        .sort({ createdAt: -1 }); // Show the newest transactions first
